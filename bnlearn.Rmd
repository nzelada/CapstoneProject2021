...
```{r}
#Now lets do last hour of the day before.
dataCovidHR=read.csv('/Users/nickzelada/Desktop/Capstone/anaPlusSPX8.csv')
```
...
# I also factor the daily mean of Polarity.
# I factor the close 
dataCovidHR$Close = factor(dataCovidHR$Close)
dataCovidHR$PerSPX = factor(dataCovidHR$PerSPX)
dataCovidHR$PerCovid = factor(dataCovidHR$PerCovid)
#grainger takes two time series, sees
```
```{r}
# Now I created the nodes.
dag4 <- empty.graph(nodes = c("PerCovid","PerSPX","Close"))
```
```{r}
arc.set4 <- matrix(c("Close", "PerCovid",
                    "Close","PerSPX"),
                  byrow = TRUE, ncol = 2,
                  dimnames = list(NULL, c("from", "to")))
```
...
```
```{r}
graphviz.plot(dag4, layout = "dot")
```
```{r}
dag4
```
```{r}
dataCovidHR = dataCovidHR[-1:-12]
```
```{r}
dataCovidHR = dataCovidHR[-2:-3]
```
...
# Find the maximum likelihood of something happening based on the frequency of it.
# 
bn.mle4 <- bn.fit(dag4, dataCovidHR, method = "mle")
```
```{r}
bn.mle4
```
...
# it takes to account everything into account. mle is different in the sense, that
# it goes by frequency for each relationship, not the whole prior data.
bn.bayes4 <- bn.fit(dag4, dataCovidHR, method = "bayes")
```
```{r}
...
#Null Hypothesis: NegPercent is probabilistically independent from Close
#Alternative: NegPercent isn't probabilistically independent from Close
ci.test("Close", c("PerSPX"), test = "mi", data = dataCovidHR)
#dependence relation- ship encoded by Close and PosPercent is not significant 
#given the current DAG structure
```
```{r}
#Here is a chart of each arc and their "p-value"
arc.strength(dag4, data = dataCovidHR, criterion = "mi")
```
```{r}
#Unlike conditional independence tests, network scores focus on the DAG as a whole
#Bayesian Information criterion
score(dag4, data = dataCovidHR, type = "bic")
```
```{r}
library(gRain)
```
```{r}
junction4 <- compile(as.grain(bn.mle4))
```
```{r}
junction4
```
```{r}
jsex4 <- setEvidence(junction4, nodes = "Close", states = "INCREASE")
```
```{r}
querygrain(jsex4, nodes = "PerSPX")$PerSPX
```
```{r}
bn.fit.barchart(bn.bayes4$PerSPX,main = "Probabilities for PerSPX on Close",
                xlab = "Pr(PerSPX|Close)", ylab = "PerSPX daily means")
