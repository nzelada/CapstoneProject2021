...
```
```{r}
RateChange = round(100*diff(anaPlusSPX$sum)/anaPlusSPX[nrow(anaPlusSPX),]$sum)
```
```{r}
RateChange = c(NA,RateChange)
```
```{r}
anaPlusSPX$RateChange = RateChange
```
```{r}
...
```
```{r fig.width = 10}
qw2<-ggplot(filtered2)+geom_line(aes(x=Date,y=sum, group = 1, color = "Tweets per Day"))+ 
  geom_line(aes(x=Date,y=percent*70, group=Analysis, color=Analysis ))+ 
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~./70, name="Percent")
  )
#daily percentage of tweets by group
qw2
```
...
ggplot(anaPlusSPXJune) +
  ...
  geom_line( aes(x=Date,y=NegPercent*150, group = 1, color = "Negative Percentage"))+ 
...
ggplot(anaPlusSPXJune) +
  ...
  geom_line( aes(x=Date,y=Close, group = 1, color = "Closing Price SPX")) + 
  geom_line( aes(x=Date,y=PosPercent*55, group = 1, color = "Positive Percentage"))+ 
...
ggplot(anaPlusSPXJune) +
  ...
  geom_line( aes(x=Date,y=Close, group = 1, color = "Closing Price SPX")) + 
  geom_line( aes(x=Date,y=Polarity*8500, group = 1, color = "Polarity Daily Mean"))+ 
...
library(ggpubr)
anaPlusSPXJune %>%
  ggplot(aes(x=NegPercent,y=PosPercent)) +
  geom_point(alpha=0.5) +
  labs(x= "NegPercent", y="PosPercent")+
  geom_smooth(method=lm)+
  stat_cor(method="pearson") 
...
```
```{r}
# for the AfterClose_BeforeOpen
AfterClose_BeforeOpen=read.csv('/Users/nickzelada/Desktop/Capstone/AfterClose_BeforeOpen.csv')
```
```{r}
AfterClose_BeforeOpen$Polarity[AfterClose_BeforeOpen$Analysis == 'Positive'] <- 1
AfterClose_BeforeOpen$Polarity[AfterClose_BeforeOpen$Analysis == 'Neutral'] <- 0
AfterClose_BeforeOpen$Polarity[AfterClose_BeforeOpen$Analysis == 'Negative'] <- -1

```

```{r}
require(data.table)
result8 <- aggregate(Polarity~created_at, data= AfterClose_BeforeOpen, FUN='mean')
```


```{r}
AfterClose_BeforeOpen$created_at<-substr(AfterClose_BeforeOpen$created_at,1,10)
```
```{r}
AfterClose_BeforeOpen1 = AfterClose_BeforeOpen%>% group_by(created_at, Analysis)%>%count()
```
```{r}
anaNeu9 = subset(AfterClose_BeforeOpen1, Analysis == "Neutral")
anaNeg9 = subset(AfterClose_BeforeOpen1, Analysis == "Negative")
anaPos9 = subset(AfterClose_BeforeOpen1, Analysis == "Positive")


```
```{r}
colnames(anaNeu9)[3] <- "Neutral"
colnames(anaNeg9)[3] <- "Negative"
colnames(anaPos9)[3] <- "Positive"
...
```
```{r}
filteredAfterClose_BeforeOpen <- aggregate(Polarity~created_at, data= AfterClose_BeforeOpen, FUN='mean')
```
```{r}
ana9 = merge(anaNeg9, anaNeu9)
ana9 = merge(ana9,anaPos9)
ana9 = merge(ana9,filteredAfterClose_BeforeOpen)
```
```{r}
ana9$sum=ana9$Negative+ana9$Neutral+ana9$Positive
```
```{r}

ana9$NegPercent = round((ana9$Negative/ana9$sum)*100)
ana9$NeuPercent = round((ana9$Neutral/ana9$sum)*100)
ana9$PosPercent = round((ana9$Positive/ana9$sum)*100)
```
```{r}
colnames(ana9)[1] <- "Date"
#anaPlusSPX = inner_join(ana,spx, by= )
```
```{r}
AfterClose_BeforeOpenPlusSPX = inner_join(ana9,spx, by= "Date" )
```
```{r fig.width = 10}
library(ggplot2)
library(ggthemes)
...
graph11=ggplot(data=AfterClose_BeforeOpenPlusSPX , aes(x = Date, y = Polarity))+
  geom_path(aes(group=1))+geom_point()
...
```{r}
allPolarity=summary(lm(AfterClose_BeforeOpenPlusSPX$Close~AfterClose_BeforeOpenPlusSPX$Polarity))
allPolarity
```
```{r}
library(ggpubr)
AfterClose_BeforeOpenPlusSPX %>%
  ggplot(aes(x=Polarity,y=Close)) +
  geom_point(alpha=0.5) +
  labs(x= "Polarity", y="Close")+
  geom_smooth(method=lm)+
...
```
```{r}
allNeg=summary(lm(AfterClose_BeforeOpenPlusSPX$Close~AfterClose_BeforeOpenPlusSPX$NegPercent))
allNeg
```
```{r}
library(ggpubr)
...
```{r}
allPos=summary(lm(AfterClose_BeforeOpenPlusSPX$Close~AfterClose_BeforeOpenPlusSPX$PosPercent))
allPos
```
```{r}
library(ggpubr)
...
```{r}
allNeu=summary(lm(AfterClose_BeforeOpenPlusSPX$Close~AfterClose_BeforeOpenPlusSPX$NeuPercent))
allNeu
```
```{r}
library(ggpubr)
...
```
```{r}
allAll=lm(AfterClose_BeforeOpenPlusSPX$Close~AfterClose_BeforeOpenPlusSPX$NeuPercent+AfterClose_BeforeOpenPlusSPX$NegPercent+AfterClose_BeforeOpenPlusSPX$PosPercent)
summary(allAll)
```

```{r fig.width = 10}
ggplot(AfterClose_BeforeOpenPlusSPX) +
...
```{r}
#Group it by day and analysis
groupAn = AfterClose_BeforeOpen%>% group_by(created_at, Analysis)%>%count()
groupAn
```
```{r}
colnames(groupAn)[1] <- "Date"
#filtered2$sum = inner_join(filtered2,sum, by= "created_at" )
```
```{r}
sum1=AfterClose_BeforeOpenPlusSPX
#filtered2$sum = inner_join(filtered2,anaPlusSPX$sum, copy= TRUE,by= "Date" )
```
```{r}
sum1=sum1[-2:-5]
...
```{r}
groupAn = inner_join(groupAn,sum1,by= "Date" )
```
```{r}
groupAn$percent=round((groupAn$n/groupAn$sum)*100)
```
```{r fig.width = 10}
qw3<-ggplot(groupAn)+geom_line(aes(x=Date,y=sum, group = 1, color = "Tweets per Day"))+ 
  geom_line(aes(x=Date,y=percent*50, group=Analysis, color=Analysis ))+ 
  ...
  scale_y_continuous(
    ...
    # Add a second axis and specify its features
    sec.axis = sec_axis(~./50, name="Percent")
  )
#daily percentage of tweets by group
qw3
```
